...
import interfaces.CrudIngresoInterface;
...
public class IngresoDAO implements CrudIngresoInterface<Ingreso, DetalleIngreso> {
    ...
    public List<Ingreso> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        try {
            ps = CON.conectar().prepareStatement("SELECT i.id,i.usuario_id,u.nombre as usuario_nombre,i.trabajador_id,t.nombre as trabajador_nombre,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.fecha,i.estado FROM ingreso i INNER JOIN trabajador t ON i.trabajador_id=t.id INNER JOIN usuario u ON i.usuario_id=u.id WHERE i.num_comprobante LIKE ? ORDER BY i.id ASC LIMIT ?,?");
            ps.setString(1, "%" + texto + "%");
            ps.setInt(2, (numPagina - 1) * totalPorPagina);
            ps.setInt(3, totalPorPagina);
            rs = ps.executeQuery();
            while (rs.next()) {
                registros.add(new Ingreso(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getInt(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getDate(9), rs.getString(10)));
            }
            ps.close();
            rs.close();
    ...
    public List<DetalleIngreso> listarDetalle(int id) {
        ...
        try {
            ps = CON.conectar().prepareStatement("SELECT a.id,a.codigo,a.nombre,d.cantidad FROM detalle_ingreso d INNER JOIN deposito a ON d.deposito_id=a.id WHERE d.ingreso_id=?");
            ps.setInt(1, id);
            rs = ps.executeQuery();
            while (rs.next()) {
                registros.add(new DetalleIngreso(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4)));
            }
            ps.close();
            rs.close();
        } catch (SQLException e) {
    ...
    public boolean insertar(Ingreso obj) {
        ...
        try {
            ...
            String sqlInsertIngreso = "INSERT INTO ingreso (trabajador_id,usuario_id,fecha,tipo_comprobante,serie_comprobante,num_comprobante,estado) VALUES (?,?,now(),?,?,?,?)";
            ...
            if (filasAfectadas == 1) {
                String sqlInsertDetalle = "INSERT INTO detalle_ingreso (ingreso_id,deposito_id,cantidad) VALUES (?,?,?)";
